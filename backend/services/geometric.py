...
def rotate(img: np.ndarray, angle: float) -> np.ndarray:
    ...
    M = cv2.getRotationMatrix2D(center, -angle, 1.0) # Gunakan -angle agar putaran searah jarum jam (CW) sesuai UI
...
def translate(img: np.ndarray, tx: float, ty: float) -> np.ndarray:
    ...
    translated = cv2.warpAffine(img, M, (w, h), flags=cv2.INTER_LINEAR, borderMode=cv2.BORDER_CONSTANT, borderValue=(0, 0, 0))
    return translated
